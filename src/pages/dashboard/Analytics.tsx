...
const analyticsSections = {
  users: { title: "Users Analytics", subtitle: "Growth and engagement across users." },
  attendance: { title: "Attendance Analytics", subtitle: "Presence trends and peak attendance windows." },
  devices: { title: "Devices Analytics", subtitle: "Device distribution and scan method performance." },
  subscription: { title: "Subscription Analytics", subtitle: "Plan usage and conversion insights." },
};
...
const Analytics = () => {
  ...
  const section = useMemo(
    () => analyticsSections[currentView as keyof typeof analyticsSections] ?? analyticsSections.users,
    [currentView],
  );
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-heading font-bold text-foreground flex items-center gap-2">
          <BarChart3 className="h-6 w-6 text-primary" /> {section.title}
        </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: "Total Users", value: "2,406" },
          { label: "Attendance", value: "92%" },
          { label: "Active Devices", value: "18" },
          { label: "Trial Conversion", value: "36%" },
        ].map((item) => (
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              <AreaChart data={monthlyData}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                ...
                <Area type="monotone" dataKey="attendance" stroke="hsl(var(--primary))" fill="hsl(var(--primary))" fillOpacity={0.12} />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
